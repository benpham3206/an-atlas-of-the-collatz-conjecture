...
**Odds: high for `N = 4–5`, uncertain past that.** Counts grow roughly like
`(N²)^N · 2^N · N`, so `N = 4` is about 4 × 10⁶ before reduction and `N = 5`
about 10⁹. Most candidates die instantly on density or periodicity; today's
data suggests survivors are a small fraction.

**Kill criterion.** Measure the exact complexity constant against
`α/(β−α)` at `N = 3` and `N = 4` first. If the constant grows faster than the
threshold shrinks, the method saturates and no compute budget rescues it —
stop and report the saturation point rather than buying more `N`.

**This is the highest expected value on the list.**
...
**Target 3**, and not target 1 or 2 — because target 3 is the only entry whose
machinery is already proved, whose failure mode is informative (a measured
saturation point for the complexity method), and which produces a stated
theorem with a number in it either way. Targets 1 and 2 are more important and
should be attempted *after* 3 has fixed how far the complexity method reaches,
since that measurement tells you whether target 2 is worth attacking at all.

Run target 3's kill criterion — the `N = 3, 4` saturation measurement — before
committing any compute to the enumeration.
