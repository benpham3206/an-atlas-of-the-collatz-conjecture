...
Tao's proof of his Proposition 1.17 proceeds through a two-dimensional
renewal process on triangles; the exact self-similar recursion of
Theorem 1, the parity-class contraction of Lemma 2, and the \(L^2\)
mixing bound of Corollary 3 do not appear in his paper, where only the
final \(n^{-A}\) bound is needed. Siegel's spectral theorem
(arXiv:2412.02902) is non-Archimedean — the Fourier transforms there
are \(C_q\)-valued and serve a Tauberian density criterion, not
Archimedean decay — and no moment machinery for \(\varphi_3\) appears
in his work. Theorem 4 (the spectral barrier) closes the naive
\(L^2\)-to-uniform route explicitly; that this route fails is implicit
in the structure of Tao's argument but is stated and verified here as a
standalone obstruction with exact eigenvalues.
